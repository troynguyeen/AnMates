...
### Global Shared Memory (`/memory/*.md`)
...
### Agent Working Contexts (`/memory/agents/<role>/CONTEXT.md`)

- **Flutter Frontend Agent** — Riverpod patterns, design conformance workflow
- **Go Backend Agent** — Gin, sqlc, PostGIS, trust ledger discipline
- **QA Agent** — Visual conformance, contract testing, regression

### Task Memory (`/memory/tasks/`)

Created per task during execution. Pattern:
- `{TASK-ID}/` — agent scratchpad while working
- `{TASK-ID}-result.md` — completion report (deliverables, blockers, notes)
- `{FE-TASK-ID}/conformance-<NN>.md` — visual diff report vs design HTML
...
### 4. Agent executes
- Creates `/memory/tasks/{TASK-ID}/` scratchpad
- Implements per spec
- Writes tests
- Submits PR / change set

### 5. Agent reports back
Writes `/memory/tasks/{TASK-ID}-result.md`:
```markdown
# {TASK-ID} Result
...
**Required context:**
  - /memory/api-contracts-summary.md §Auth
  - /memory/shared-decisions.md §Decision 2
**Forbidden context:**
...
├── api-contracts-summary.md          # Endpoint catalog
...
├── shared-decisions.md               # 15 locked decisions
├── task-board.md                     # ~160 tasks scaffolded
├── agents/
│   ├── flutter/FE-CONTEXT.md
│   ├── backend/BE-CONTEXT.md
│   └── qa/QA-CONTEXT.md
└── tasks/                            # ephemeral per-task memory
```
